...
function connected(socket) {
	...
	socket.on("join", (data) => {
		...
		socket.join(lobby);
		console.log(`New client connection: ${Lib.clientNo.number}, room nr. ${lobby} (${socket.id})`);
		let senddata = {
			room: lobby,
			client: Lib.clientNo.number,
			name: data.name,
			icon: false,
		};
	...
	socket.on("create", (data) => {
		...
		Lib.roomNo[data] = data;
			...
			console.log(
				`New client connection: ${Lib.clientNo.number}, room nr. ${Lib.roomNo[data]} (${socket.id})`
			);
			let datacreate = {
	...
	socket.on("addContentToDb", (senddata) => {
		...
		Lib.io.in(senddata.data.game).emit("updateReadyPlayers", quantity.length);
	});
	...
	socket.on("disconnect", () => {
		...
		if (dcuser != undefined) {
			...
			if (dcuser.name === dcuser.lobby) {
				...
				Lib.removeAllUsersFromArray(userToRoom, dcuser);
